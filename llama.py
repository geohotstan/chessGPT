...
class LLaMAchess:
  def __init__(self, count=1000, temperature=0.7, size="7B", gen=1, quantize=True):
    self.count = count
    self.temperature = temperature
    self.model_path = Path(__file__).parent / f"weights/LLaMA/{size}" 
    self.tokenizer_path = (MODEL_PATH if MODEL_PATH.is_dir() else MODEL_PATH.parent) / "tokenizer.model"
    self.model = LLaMa.build(self.model_path, self.tokenizer_path, model_gen=gen, model_size=size, quantize=quantize)
  

if __name__ == "__main__":
  Tensor.no_grad = True
  print(f"using {Device.DEFAULT} backend")
  pre_prompt = "After you are done speaking, output [EOS]. You are not the User."
  examples = {
  ...
  LLAMA_SUFFIX = "" # "-2" if Llama 2
  MODEL_PATH = Path(__file__).parent / f"weights/LLaMA/7B"
